...
import com.chatlive.chatLive.mappers.MessageMapper;
...
import com.chatlive.chatLive.repositories.MessageRepository;
@Service
@Transactional
...
public class MessageService {
    ...
    @Transactional
    public MessageDto sendMessageAndUpdateUnread(Long chatId, Integer senderId, String content, String messageType, Long repliedMessageId) {
        ...
        for (ChatUser chatUser: chat.getParticipants()){
            pushNotificationService.getSubscription(chatUser.getUser()).ifPresent(subscription -> {
                try {
                    pushNotificationService.sendNotification(subscription, "Nuevo mensaje", message.getContent());
    ...
    public void deleteMessageAll(Long messageId, Integer userId) {
    ...
    public List<MessageDto> changeMessagesStatus(List<Long> messageIds, MessageStatus newStatus) {
        ...
        messages.forEach(message -> {
            ...
            if (newStatus.equals(MessageStatus.SEEN)) message.setSeenAt(LocalDateTime.now());
            if (newStatus.equals(MessageStatus.DELIVERED)) message.setDeliveredAt(LocalDateTime.now());
        });
    ...
    public void markAllMessagesAsDeliveredForAuthenticatedUser(MessageStatus newStatus) {
        ...
        messagesByChat.forEach((chatId, messages) ->
                messagingTemplate.convertAndSend("/topic/chat/" + chatId + "/updateStatus/message", messages.
